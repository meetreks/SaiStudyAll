...
class Head(beam.DoFn):
    def __init__(self):
        self.empty_line_counter = Metrics.distribution('main','empty_lines')

    def process(self, element):

        if (element!= skip_head):
            #self.empty_line_counter.update()
            yield element
...
class Split(beam.DoFn):
    ...
    def process(self, element):
        ...
        if(y == "sedan") or (y == "hatch"):
            #self.word_length_counter.update(y)
            yield element

class Filter(beam.DoFn):
    def __init__(self):
        self.word_counter = Metrics.distribution('main','total_words')
